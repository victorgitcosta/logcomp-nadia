...
class Lexer():
    ...
    def _add_tokens(self):
        ...
        ######################################################################################################################################################
        # LOG.COMP 2025.02
        self.lexer.add('IR1', r'ir1')
        self.lexer.add('IR2', r'ir2')
        self.lexer.add('IR3', r'ir3')
        self.lexer.add('IR4', r'ir4')
        self.lexer.add('IR5', r'ir5')
        self.lexer.add('IR6', r'ir6')
        self.lexer.add('ER1', r'er1')
        self.lexer.add('ER2', r'er2')
        self.lexer.add('ER3', r'er3')
        self.lexer.add('ER4', r'er4')
        self.lexer.add('ER5', r'er5')
        self.lexer.add('ER6', r'er6')
        ######################################################################################################################################################
        #Comma
...
class constants:
    ...
    # LOG. COMP 2025.02
    INVALID_INTRODUCTION_RULE = 48
    INVALID_ELIMINATION_RULE = 49
...
class IntroductionRule1Def():
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if formula1 is None or formula_reference is None:
            return

        if not isinstance(formula1, NegationFormula):
            deduction_result.add_error(
                "A fórmula referenciada na linha {} deve ser uma negação (~P).".format(self.reference1.value)
            )
            return

        if not isinstance(self.formula, NegationFormula):
            deduction_result.add_error(
                "A conclusão na linha {} deve ser uma negação da forma ~(P & Q).".format(self.line)
            )
            return
        
        inner_formula = self.formula.formula
        if not isinstance(inner_formula, AndFormula):
            deduction_result.add_error(
                "O conteúdo da negação na linha {} deve ser uma conjunção (P & Q).".format(self.line)
            )
            return

        A = formula1.formula  # Premissa: ~P
        B = inner_formula.left # ~P em |- ~(P & Q)

        if A != B:
            deduction_result.add_error(
                "A fórmula negada na linha {} ({}) não corresponde ao termo esquerdo da conjunção na conclusão.".format(
                    self.reference1.value, A.toString()
                )
            )
            return

    def toLatex(self, symbol_table):
        return '\\infer[\\text{IR1}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'

class IntroductionRule2Def():
    """
    Regra: ~Q |- ~(P & Q)
    """
    def __init__(self, line, formula, reference1):
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if not isinstance(formula1, NegationFormula):
            deduction_result.add_error(
                "A fórmula referenciada na linha {} deve ser uma negação (~Q).".format(self.reference1.value)
            )
            return
        ...
        A = formula1.formula
        B = inner_formula.right

        if A != B:
            deduction_result.add_error(
                "A fórmula negada na linha {} ({}) não corresponde ao termo direito da conjunção na conclusão.".format(
                    self.reference1.value, A.toString()
                )
            )
            return
    ...
    def toLatex(self, symbol_table):
        return '\\infer[\\text{IR2}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'

class IntroductionRule3Def():
    """
    Regra: ~P, ~Q |- ~(P v Q)
    """
    def __init__(self, line, formula, reference1, reference2):
    ...
    def evaluation(self, parser, deduction_result):
        before1 = parser.check_line_reference_before_rule_error(deduction_result, self)
        if before1:
            parser.check_line_scope_reference_error(deduction_result, self, reference1=True)
        
        if self.reference2:
             if self.reference2.value >= self.line:
                 deduction_result.add_error(f"Erro na linha {self.line}: Referência {self.reference2.value} inválida.")
                 return

        formula_ref1 = parser.symbol_table.lookup_formula_by_line(self.line, self.reference1.value)
        formula_ref2 = parser.symbol_table.lookup_formula_by_line(self.line, self.reference2.value)

        if formula_ref1 is None or formula_ref2 is None:
            return

        if not isinstance(self.formula, NegationFormula):
            deduction_result.add_error(
                "A conclusão na linha {} deve ser uma negação da forma ~(P v Q).".format(self.line)
            )
            return

        inner_formula = self.formula.formula
        
        if not isinstance(inner_formula, OrFormula): 
            deduction_result.add_error(
                "O conteúdo da negação na linha {} deve ser uma disjunção (P v Q).".format(self.line)
            )
            return

        A = NegationFormula(inner_formula.left)
        B = NegationFormula(inner_formula.right)

        # Cenário 1: Ref1 é ~P e Ref2 é ~Q
        match_order_1 = (formula_ref1 == A) and (formula_ref2 == B)
        
        # Cenário 2: Ref1 é ~Q e Ref2 é ~P
        match_order_2 = (formula_ref1 == B) and (formula_ref2 == A)

        if not (match_order_1 or match_order_2):
            deduction_result.add_error(
                "As referências nas linhas {} e {} não correspondem às negações das partes da disjunção {}.".format(
                    self.reference1.value, self.reference2.value, inner_formula.toString()
                )
            )
            return

    def toLatex(self, symbol_table):
        ref1_latex = symbol_table.get_rule(self.reference1.value).toLatex(symbol_table)
        ref2_latex = symbol_table.get_rule(self.reference2.value).toLatex(symbol_table)
        return '\\infer[\\text{IR3}]{' + self.formula.toLatex() + '}{' + ref1_latex + ' & ' + ref2_latex + '}'
class IntroductionRule4Def():
    """
    Regra: P, ~Q |- ~(P -> Q)
    """
    def __init__(self, line, formula, reference1, reference2):
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if not isinstance(inner_formula, ImplicationFormula):
            deduction_result.add_error(
                "O conteúdo da negação na linha {} deve ser uma implicação (P -> Q).".format(self.line)
            )
            return

        A = inner_formula.left
        B = NegationFormula(inner_formula.right)
        ...
        if not (match_order_1 or match_order_2):
            deduction_result.add_error(
                "Para concluir {}, as referências devem ser o antecedente ({}) e a negação do consequente ({}).".format(
                    self.formula.toString(), A.toString(), B.toString()
                )
            )
            return

    def toLatex(self, symbol_table):
        return '\\infer[\\text{IR4}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + ' & ' + symbol_table.get_rule(self.reference2.value).toLatex(symbol_table) + '}'
    
class IntroductionRule5Def():
    """
    Regra: ~P |- P -> Q
    """
    def __init__(self, line, formula, reference1):
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if not isinstance(self.formula, ImplicationFormula):
            deduction_result.add_error(
                "A conclusão na linha {} deve ser uma implicação (P -> Q).".format(self.line)
            )
            return
        ...
        if A != B:
            deduction_result.add_error(
                "O antecedente da implicação na linha {} ({}) não corresponde à fórmula negada na linha {} ({}).".format(
                    self.line, B.toString(), self.reference1.value, A.toString()
                )
            )
            return

    def toLatex(self, symbol_table):
        return '\\infer[\\text{IR5}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'

class IntroductionRule6Def():
    """
    Regra: Q |- P -> Q
    """
    def __init__(self, line, formula, reference1):
    ...
    def evaluation(self, parser, deduction_result):
        ...
        A = formula1
        B = self.formula.right 

        if A != B:
            deduction_result.add_error(
                "O consequente da implicação na linha {} ({}) não corresponde à fórmula referenciada na linha {} ({}).".format(
                    self.line, B.toString(), self.reference1.value, A.toString()
                )
            )
            return
    ...
    def toLatex(self, symbol_table):
        return '\\infer[\\text{TrueCons}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'

class EliminationRule1Def():
    """
    Regra: ~(P | Q) |- ~P
    """
    def __init__(self, line, formula, reference1):
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if not isinstance(formula1, NegationFormula):
            deduction_result.add_error(
                "A fórmula referenciada na linha {} deve ser uma negação da forma ~(P | Q).".format(self.reference1.value)
            )
            return

        inner_premise = formula1.formula
        if not isinstance(inner_premise, OrFormula):
            deduction_result.add_error(
                "O conteúdo da negação na premissa {} deve ser uma disjunção (P | Q).".format(self.reference1.value)
            )
            return

        if not isinstance(self.formula, NegationFormula):
            deduction_result.add_error(
                "A conclusão na linha {} deve ser uma negação (~P).".format(self.line)
            )
            return
        
        P_esperado = inner_premise.left
        P_conclusao = self.formula.formula

        if P_conclusao != P_esperado:
            deduction_result.add_error(
                "A fórmula negada na conclusão ({}) não corresponde ao termo esquerdo da disjunção negada na premissa ({}).".format(
                    P_conclusao.toString(), P_esperado.toString()
                )
            )
            return

    def toLatex(self, symbol_table):
        return '\\infer[\\text{ER1}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'

class EliminationRule2Def():
    """
    Regra: ~(P | Q) |- ~Q
    """
    def __init__(self, line, formula, reference1):
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if not isinstance(self.formula, NegationFormula):
            deduction_result.add_error(
                "A conclusão na linha {} deve ser uma negação (~Q).".format(self.line)
            )
            return

        Q_esperado = inner_premise.right
        Q_conclusao = self.formula.formula

        if Q_conclusao != Q_esperado:
            deduction_result.add_error(
                "A fórmula negada na conclusão ({}) não corresponde ao termo direito da disjunção negada na premissa ({}).".format(
                    Q_conclusao.toString(), Q_esperado.toString()
                )
            )
            return

    def toLatex(self, symbol_table):
        return '\\infer[\\text{ER2}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'
    
class EliminationRule3Def():
    """
    Regra: ~(P & Q), P |- ~Q
    """
    def __init__(self, line, formula, reference1, reference2):
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if self.reference2:
             if self.reference2.value >= self.line:
                 deduction_result.add_error(f"Erro na linha {self.line}: Referência {self.reference2.value} inválida (deve vir antes da linha atual).")
                 return
        ...
        #Variáveis vazias para depois preencher de acordo com a ordem que as premissas foram colocadas no Nadia
        neg_and_formula = None
        affirmation_formula = None
        
        def is_negated_and(f):
            return isinstance(f, NegationFormula) and isinstance(f.formula, AndFormula)

        if is_negated_and(formula_ref1):
            neg_and_formula = formula_ref1
            affirmation_formula = formula_ref2
        elif is_negated_and(formula_ref2):
            neg_and_formula = formula_ref2
            affirmation_formula = formula_ref1
        else:
            deduction_result.add_error(
                "Pelo menos uma das referências deve ser a negação de uma conjunção na forma ~(P & Q)."
            )
            return

        A = neg_and_formula.formula.left
        B = neg_and_formula.formula.right

        if affirmation_formula != A:
            deduction_result.add_error(
                "A premissa afirmativa ({}) não corresponde ao termo esquerdo da conjunção negada ({}).".format(
                    affirmation_formula.toString(), A.toString()
                )
            )
            return
        ...
        if Q_conclusao != B:
            deduction_result.add_error(
                "A conclusão ({}) não corresponde à negação do termo direito da conjunção ({}).".format(
                    self.formula.toString(), B.toString()
                )
            )
            return

    def toLatex(self, symbol_table):
        return '\\infer[\\text{ER3}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + ' & ' + symbol_table.get_rule(self.reference2.value).toLatex(symbol_table) + '}'
    
class EliminationRule4Def():
    """
    Regra: ~(P & Q), Q |- ~P
    """
    def __init__(self, line, formula, reference1, reference2):
    ...
    def evaluation(self, parser, deduction_result):
        ...
        P_esperado = neg_and_formula.formula.left
        Q_esperado = neg_and_formula.formula.right

        # 5. Validação da Premissa Afirmativa (Q)
        # Diferença crucial para a regra anterior: A afirmação deve bater com o lado DIREITO.
        if affirmation_formula != Q_esperado:
            deduction_result.add_error(
                "A premissa afirmativa ({}) não corresponde ao termo direito da conjunção negada ({}).".format(
                    affirmation_formula.toString(), Q_esperado.toString()
                )
            )
            return

        # 6. Validação da Conclusão (~P)
        # 6.1 Deve ser negação
        if not isinstance(self.formula, NegationFormula):
            deduction_result.add_error(
                "A conclusão na linha {} deve ser uma negação (~P).".format(self.line)
            )
        ...
        # 6.2 O conteúdo deve ser o lado ESQUERDO (P)
        P_conclusao = self.formula.formula
        
        if P_conclusao != P_esperado:
            deduction_result.add_error(
                "A conclusão ({}) não corresponde à negação do termo esquerdo da conjunção ({}).".format(
                    self.formula.toString(), P_esperado.toString()
                )
            )
            return

    def toLatex(self, symbol_table):
        return '\\infer[\\text{ER4}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + ' & ' + symbol_table.get_rule(self.reference2.value).toLatex(symbol_table) + '}'

class EliminationRule5Def():
    """
    Regra~(P -> Q) |- P
    """
    def __init__(self, line, formula, reference1):
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if not isinstance(formula1, NegationFormula):
            deduction_result.add_error(
                "A fórmula referenciada na linha {} deve ser uma negação (~(P -> Q)).".format(self.reference1.value)
            )
            return

        inner_formula = formula1.formula
        if not isinstance(inner_formula, ImplicationFormula):
            deduction_result.add_error(
                "O conteúdo da negação na premissa {} deve ser uma implicação (P -> Q).".format(self.reference1.value)
            )
            return

        P_expected = inner_formula.left
        P_conclusao = self.formula

        if P_conclusao != P_expected:
            deduction_result.add_error(
                "A conclusão ({}) não corresponde ao antecedente da implicação negada na premissa ({}).".format(
                    P_conclusao.toString(), P_expected.toString()
                )
            )
            return

    def toLatex(self, symbol_table):
        return '\\infer[\\text{ER5}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'
    
class EliminationRule6Def():
    """
    Regra: ~(P -> Q) |- ~Q
    """
    def __init__(self, line, formula, reference1):
    ...
    def evaluation(self, parser, deduction_result):
        ...
        A = inner_formula.right
        B = self.formula.formula

        if B != A:
            deduction_result.add_error(
                "A fórmula negada na conclusão ({}) não corresponde ao consequente da implicação negada na premissa ({}).".format(
    ...
    def toLatex(self, symbol_table):
        return '\\infer[\\text{ER6}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'
######################################################################################################################################################
# LOGICOMP
class DeMorgan1Def:
    """¬(A ∧ B) ⊢ ¬A ∨ ¬B"""
    ...
    def __init__(self, state):
        ...
        self.pg = ParserGenerator(
            ...
            ['NUM', 'DOT', 'COMMA', 'OPEN_PAREN', 'CLOSE_PAREN', 'NOT', 'RAA',
             ...
             'IR1', 'IR2', 'IR3', 'IR4', 'IR5', 'IR6', 'ER1', 'ER2', 'ER3', 'ER4', 'ER5', 'ER6'],
    ...
    def parse(self):
        ...
        def program(p):
            ...
            for i in rule_info:
                ...
                ######################################################################################################################################################
                # LOG.COMP 2025.02
                elif isinstance(rule, IntroductionRule1Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, IntroductionRule2Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, IntroductionRule3Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, IntroductionRule4Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, IntroductionRule5Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, IntroductionRule6Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, EliminationRule1Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, EliminationRule2Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, EliminationRule3Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, EliminationRule4Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, EliminationRule5Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, EliminationRule6Def):
                    rule.evaluation(self, deduction_result)
                ######################################################################################################################################################
...
#            return p[0], formula_result[0]
        ...
        @self.pg.production('step : NUM DOT formula IR1 NUM')
        def IntroductionRule1(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = IntroductionRule1Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & IR1 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]
        
        @self.pg.production('step : NUM DOT formula IR2 NUM')
        def IntroductionRule2(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = IntroductionRule2Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & IR2 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]
        
        @self.pg.production('step : NUM DOT formula IR3 NUM COMMA NUM')
        def IntroductionRule3(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = IntroductionRule3Def(p[0].value, formula, p[4], p[6])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & IR3 {}, {}\\\\\n".format(formula.toLatex(), p[4].value, p[6].value)
            return p[0], formula_result[0]
        
        @self.pg.production('step : NUM DOT formula IR4 NUM COMMA NUM')
        def IntroductionRule4(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = IntroductionRule4Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & IR4 {}, {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]
        
        @self.pg.production('step : NUM DOT formula IR5 NUM')
        def IntroductionRule5(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = IntroductionRule5Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & IR5 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]
        
        @self.pg.production('step : NUM DOT formula IR6 NUM')
        def IntroductionRule6(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = IntroductionRule6Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & IR6 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]
        
        @self.pg.production('step : NUM DOT formula ER1 NUM')
        def EliminationRule1(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = EliminationRule1Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & ER1 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]
        
        @self.pg.production('step : NUM DOT formula ER2 NUM')
        def EliminationRule2(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = EliminationRule2Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & ER2 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]
        @self.pg.production('step : NUM DOT formula ER3 NUM')
        def EliminationRule3(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = EliminationRule3Def(p[0].value, formula, p[4], p[6])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & ER3 {}, {}\\\\\n".format(formula.toLatex(), p[4].value, p[6].value)
            return p[0], formula_result[0]
        @self.pg.production('step : NUM DOT formula ER4 NUM')
        def EliminationRule4(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = EliminationRule4Def(p[0].value, formula, p[4], p[6])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & ER4 {}, {}\\\\\n".format(formula.toLatex(), p[4].value, p[6].value)
            return p[0], formula_result[0]
        
        @self.pg.production('step : NUM DOT formula ER5 NUM')
        def EliminationRule5(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = EliminationRule5Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & ER5 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]
        
        @self.pg.production('step : NUM DOT formula ER6 NUM')
        def EliminationRule6(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = EliminationRule6Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & ER6 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]
